		...
		if "tile" in tag:
			...
			if tile_coords[ 0 ] >= enemy_limits[ 0 ] and tile_coords[ 0 ] < enemy_limits[ 1 ]:
				...
				if crossFreeTile( tile_coords[ 0 ], tile_coords[ 1 ] ):
					...
					ctCreateObject( cross.tag, cross.image, position = coordToPosition( cross.position[ 0 ], cross.position[ 1 ] ), size = ( SIZE, SIZE ) )
...
ctBindFunctions( start, update, key_pressed = keyPressed, mouse_moved = mouseMoved, ray_cast = raycast, ray_cast_entered = raycastEntered, ray_cast_exited =raycastExited )
ctBegin()
